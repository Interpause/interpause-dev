...
export type IconProps = (({orientation?:number} & Omit<ImageProps,"height"|"width"|"layout">) | ({icon:ICON} & OrientableSVG)) & {as?:keyof JSX.IntrinsicElements,href?:string};
/** Must restyle with height and width. Either specify icon which is enum ICON or src which is path. */
export function Icon({orientation,className,href,as,...props}:IconProps){
	return <IconWrapper as={as} className={className} orientation={orientation} href={href}>
		{
			("src" in props)?
