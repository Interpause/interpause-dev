...
export function RainbowText({orientation,children,config,...props}:RainbowTextProps){
	...
	useEffect(() => {
		...
		measureSpan.innerText = children.toString();
	...
	},[children.toString(),props.className]);
	...
	return (
		<svg ref={fontRef} transform={`rotate(${orientation??0} 0 0)`} viewBox={`0 0 ${state.width} ${state.height}`} xmlns="http://www.w3.org/2000/svg" version="1.1" {...props}>
			...
			<text x="50%" textAnchor="middle" dominantBaseline="text-before-edge" style={{font:state.font}} fill="url(#pattern)">{children}</text>
		</svg>
	);
}