...
function smart_shuffle(conf: IsogridConfig, above?: HexColor, left?: HexColor) {
  ...
  if (rand() < conf.randomness && typeof above !== 'undefined')
    frndarr.splice(frndarr.indexOf(above), 1)
  if (rand() < conf.randomness && typeof left !== 'undefined')
...
function gen_tri(conf: IsogridConfig, above?: HexColor, left?: HexColor) {
  ...
  return {
    color_seq: r_seq,
    speed:
      Math.floor(rand() * (conf.max_speed - conf.min_speed)) + conf.min_speed,
    points: '',
  } as TriangleProps
}
...
export const IsogridBackground = React.memo(
  (props?: Partial<IsogridConfig>) => {
    ...
    ;(Object.entries(props ?? {}) as [IsogridKeys, any][]).forEach(
      ([k, v]) => (conf[k] = v),
    )
