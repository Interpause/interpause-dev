...
function Header({ className }: { className?: string }) {
  return (
    <header className={className}>
      ...
      <h2 tw='text-lg lg:(text-4xl)'>
        "hazy dreams made clear by earnest coding"
      </h2>
      ...
      <SocialsBar tw='mx-auto lg:(mx-0) text-special' />{' '}
      {/* No idea what colour this should be. Give up. */}
    </header>
  )
}
      ...
      <div tw='relative h-screen w-full'>
        <IsogridBackground
          ...
          rand_seed={
            ...
            (d.getDate() + Math.PI) *
            (d.getMonth() + Math.PI) *
            (d.getMinutes() + Math.PI)
          }
        />
        <h4 tw='hidden md:block absolute z-25 bottom-4 right-2 text-lg bg-normal-soft rounded p-1'>
          <a
            tw='hocus:text-link-color'
            href='https://github.com/Interpause/interpause-dev/blob/main/components/deco/IsogridBackground.tsx'
          >
            How is this background generated?
          </a>
        </h4>
        <ScrollHint
          orientation={Orientation.up}
          tw='absolute bottom-2 mx-auto inset-x-0 h-20 text-white pointer-events-none'
          css={hideMobileLandscape}
        />
      </div>
      <div tw='absolute inset-0 flex flex-col justify-evenly'>
        <div tw='h-20 flex-auto'></div>
        <Header
          tw='flex-none text-center mx-1.5 p-2 rounded-lg lg:(ml-5 mr-auto text-left)'
          css={css`
    ...
    body: (
      <div tw='text-base'>
        <p>
          Email:
          <br />
          <a tw='text-link-color' href='mailto:hyphen@interpause.dev'>
            hyphen@interpause.dev
          </a>
        </p>
        <p>
          Github:
          <br />
          <a tw='text-link-color' href='https://github.com/interpause'>
        ...
        <p>
          LinkedIn:
          <br />
          <a tw='text-link-color' href='https://linkedin.com/in/interpause'>
            linkedin.com/in/interpause
          </a>
        </p>
      </div>
    ),
...
function Main() {
  ...
  return (
    <section
      tw='text-center min-h-screen border-t border-normal-hard'
      id='main'
    >
  ...
  return (
    <>
      ...
      <div tw='mb-4 lg:hidden' css={hideMobilePotrait}>
        <h2 tw='text-5xl py-4 font-extralight overflow-hidden'>
          Brief Overview
        </h2>
